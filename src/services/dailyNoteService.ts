...
export async function loadDailyNotes(from?: string, to?: string) { let query = supabase.from('daily_notes').select('id,date,note'); if (from) query = query.gte('date', from); if (to) query = query.lte('date', to); const { data, error } = await query.order('date'); if (error) throw error; return (data ?? []) as DailyNote[] }
