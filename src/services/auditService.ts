...
export async function loadAllAuditEvents() {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await supabase.from('security_audit_log').select('id,actor_id,action,entity_type,entity_id,created_at').order('created_at', { ascending: true }).range(from, from + 999)
