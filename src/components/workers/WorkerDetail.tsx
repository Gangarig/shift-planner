...
function WorkerDetail({ worker, onRemoveWorker, setSelectedWorker, assignments, onChangeOfStatus, updateWorkerState, absences, weekStart, onCreateAbsence, onRemoveAbsence }: WorkerDetailProps) {
  const [vacationDays, setVacationDays] = useState('')
  const [plusHours, setPlusHours] = useState('')
  const [absenceStatus, setAbsenceStatus] = useState<NewWorkerAbsence['status']>('sick')
  ...
  return <Paper withBorder p="lg"><Stack>
    ...
    <Select label="General availability" description="Use this for the whole selected week. Use a dated status below for a shorter absence." value={worker.status} data={['available', 'late', 'sick', 'holiday', 'inactive']} onChange={(value) => onChangeOfStatus({ ...worker, status: (value ?? 'available') as WorkerStatus })} />
    ...
    <Divider />
    <Stack gap="xs"><Text fw={600}>Dated absence or delay</Text><Text size="sm" c="dimmed">Sick leave and vacation remove assignments only inside this date range. Late remains scheduled.</Text><Select label="Type" value={absenceStatus} data={[{ value: 'sick', label: 'Sick' }, { value: 'holiday', label: 'Vacation' }, { value: 'late', label: 'Late' }]} onChange={value => setAbsenceStatus((value ?? 'sick') as NewWorkerAbsence['status'])} /><Group grow><TextInput type="date" label="From" value={absenceStart} onChange={event => setAbsenceStart(event.currentTarget.value)} /><TextInput type="date" label="To" value={absenceEnd} min={absenceStart} onChange={event => setAbsenceEnd(event.currentTarget.value)} /></Group><Textarea label="Note" value={absenceNote} onChange={event => setAbsenceNote(event.currentTarget.value)} maxLength={1000} autosize minRows={2} /><Button variant="light" loading={savingAbsence} disabled={!absenceStart || !absenceEnd || absenceEnd < absenceStart} onClick={async () => { setSavingAbsence(true); const ok = await onCreateAbsence({ workerId: currentWorker.id, startDate: absenceStart, endDate: absenceEnd, status: absenceStatus, note: absenceNote.trim() || null }); setSavingAbsence(false); if (ok) setAbsenceNote('') }}>Save dated status</Button>{absences.filter(item => item.workerId === currentWorker.id).map(item => <Group justify="space-between" key={item.id}><div><Badge color={item.status === 'sick' ? 'red' : 'orange'}>{item.status === 'holiday' ? 'Vacation' : item.status}</Badge><Text size="xs" c="dimmed">{item.startDate} → {item.endDate}{item.note ? ` · ${item.note}` : ''}</Text></div><Button size="xs" variant="subtle" color="red" onClick={() => void onRemoveAbsence(item.id)}>Remove</Button></Group>)}</Stack>
    <Divider />
