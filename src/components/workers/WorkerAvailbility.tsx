...
function WorkerAvailability() {
  const { workers, assignments, stations, weekDays, updateWorker, weeklyPlan, companyClosures, autoAssignPreferredWorkers, publishWeeklyPlan } = useApp()
  const { user } = useAuth()
  ...
  const weekAssignments = assignments.filter(assignment => dates.has(toDateKey(assignment.date)) && !companyClosures.some(item => item.start_date <= toDateKey(assignment.date) && item.end_date >= toDateKey(assignment.date)))
  const dateLabel = (date: Date) => date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
  const companyClosure = (date: Date) => companyClosures.find(item => item.start_date <= toDateKey(date) && item.end_date >= toDateKey(date))
      ...
      <div className="schedule-scroll compact dashboard-schedule-scroll">
        <table className="schedule-table">
          <thead><tr><th scope="col">Station</th>{weekDays.map(day => {
            const holiday = austrianPublicHoliday(day.date); const closure = companyClosure(day.date); const closedLabel = holiday ?? closure?.label
            return <th scope="col" key={toDateKey(day.date)} className={closedLabel ? 'holiday-column' : ''}>{day.label.slice(0, 3)}<span>{dateLabel(day.date)}</span>{closedLabel && <small>Closed · {closedLabel}</small>}</th>
          })}</tr></thead>
          <tbody>{stations.map(station => <tr key={station.id}>
            ...
            {weekDays.map(day => {
              ...
              const holiday = austrianPublicHoliday(day.date); const closure = companyClosure(day.date)
      ...
      <Paper withBorder p="xs" className="schedule-roster dashboard-roster">
        <Stack gap="sm"><Group justify="space-between"><Text fw={700}>Team</Text><Badge color="gray" variant="light">{workers.length}</Badge></Group>
          <div className="roster-list">{workers.map(worker => {
            ...
            const content = <><span><strong>{worker.name}</strong><small>{count} {count === 1 ? 'shift' : 'shifts'}</small></span><Badge ml="auto" size="xs" variant="dot" color={statusColors[worker.status]}>{worker.status}</Badge></>
