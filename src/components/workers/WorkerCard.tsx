...
function WorkerCard({ worker, selectedWorker, onSelectWorker, stations, onUpdateWorker }: WorkerCardProps) {
  ...
  const colors: Record<WorkerStatus, string> = { available: 'blue', late: 'orange', sick: 'red', holiday: 'yellow', inactive: 'gray' }
  ...
  return <Paper withBorder p="xs" radius="md" onClick={() => onSelectWorker(worker)} bg={isSelected ? 'var(--mantine-color-blue-light)' : undefined} style={{ cursor: 'pointer', textAlign: 'left', width: '100%', borderColor: isSelected ? 'var(--mantine-color-blue-6)' : undefined }}>
    <Group wrap="nowrap">
      ...
      <Menu shadow="md" width={220} position="bottom-end"><Menu.Target><ActionIcon aria-label={`Quick options for ${worker.name}`} variant="subtle" onClick={event => event.stopPropagation()}>•••</ActionIcon></Menu.Target>
        <Menu.Dropdown className="worker-options-menu" onClick={event => event.stopPropagation()}><Menu.Label>Status</Menu.Label>{statuses.map(status => <Menu.Item key={status} onClick={() => onUpdateWorker({ ...worker, status })}>{statusLabel(status)}{worker.status === status ? ' ✓' : ''}</Menu.Item>)}<Menu.Divider /><Menu.Label>Main station</Menu.Label><Menu.Item onClick={() => onUpdateWorker({ ...worker, preferredStationId: null })}>No main station{!worker.preferredStationId ? ' ✓' : ''}</Menu.Item>{stations.filter(station => station.active).map(station => <Menu.Item key={station.id} onClick={() => onUpdateWorker({ ...worker, preferredStationId: station.id })}>{station.name}{worker.preferredStationId === station.id ? ' ✓' : ''}</Menu.Item>)}</Menu.Dropdown>
      </Menu>
    </Group>
  </Paper>
}
