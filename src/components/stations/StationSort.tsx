...
function StationSort({sortOrderStation,onSortStation}:StationSortProps) {
  return (
    <Select aria-label="Sort stations" value={sortOrderStation}
      data={['Default', 'Name A-Z', 'Name Z-A', 'Active first', 'Inactive first']}
      onChange={(value) => value && onSortStation(value as StationSortProps['sortOrderStation'])} />
  )
}
