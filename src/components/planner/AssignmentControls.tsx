...
function AssignmentControls({ stations, workers, assignments, onCreateAssignment, onCreated, weekDays }: AssignmentControlsProps) {
  ...
  async function handleSubmit() {
    ...
    if (!worker || !station || !selectedDate || !station.active || !['available', 'late'].includes(worker.status)) return
  ...
  return <Stack gap="sm">
    ...
    <Select label="Worker" placeholder="Choose worker" searchable value={selectedWorkerId} data={workers.map((worker) => ({ value: worker.id, label: worker.name, disabled: !['available', 'late'].includes(worker.status) }))} onChange={(value) => setSelectedWorkerId(value ?? '')} />
