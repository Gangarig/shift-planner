...
function PlannerWorkerList({ workers, assignments }: PlannerWorkerListProps) {
  ...
  return <Paper withBorder p="sm" className="planner-workers"><Stack gap="sm">
    <Group justify="space-between"><Text fw={700} size="sm">Available workers</Text><Badge color="green" variant="light">{availableWorkers.length}</Badge></Group>
    <Text size="xs" c="dimmed">Drag a worker onto an empty shift cell.</Text>
    {availableWorkers.map((worker) => workerCard(worker, true))}
    {availableWorkers.length === 0 && <Text size="sm" c="dimmed">No available workers.</Text>}
    <Text fw={700} size="sm" mt="sm">Unavailable</Text>
    {unavailableStatuses.map((status) => {
      const group = unavailableWorkers.filter((worker) => worker.status === status)
      return group.length > 0 && <Stack gap={4} key={status}><Text tt="capitalize" size="xs" c="dimmed">{status}</Text>{group.map((worker) => workerCard(worker, false))}</Stack>
    })}
  </Stack></Paper>
}
